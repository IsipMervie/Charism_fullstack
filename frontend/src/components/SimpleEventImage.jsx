import React from 'react';

const SimpleEventImage = ({ event, className = '', alt = '', style = {} }) => {
  // Clean default SVG - NO NETWORK REQUESTS
  const defaultSVG = 'data:image/svg+xml;base64,';

  // SIMPLE LOGIC - NO COMPLEX CHECKS
  const getImageSrc = () => {
    if (!event || !event.image || !event.image.data || event.image.data.length === 0) {
      return defaultSVG;
    }
    return `https://charism-api-xtw9.onrender.com/api/files/event-image/${event._id}`;
  };

  return (
    <div className={`event-image-container ${className}`} style={style}>
      <img
        src={getImageSrc()}
        alt={alt || event?.title || 'Event Image'}
        className="event-image"
        style={{
          width: '100%',
          height: '100%',
          objectFit: 'cover',
          ...style
        }}
        onError={(e) => {
          e.target.src = defaultSVG;
        }}
      />
    </div>
  );
};

export default SimpleEventImage;